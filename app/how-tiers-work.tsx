...
import { cardNeeded, nextTierOf, staying, tierName } from '../src/ui/tiers';
...
export default function HowTiersWork() {
  ...
  if (!s) {
    return <Screen tab="card" back title="How Tiers Work" refreshing={refreshing} onRefresh={refresh} status={statusOf(s, failed)}>{null}</Screen>;
  }
  const stay = staying(s);
  const next = nextTierOf(s);
  const cardTier = stay ? s.tier : next ?? 'gold';
  ...
  return (
    <Screen tab="card" back title="How Tiers Work" refreshing={refreshing} onRefresh={refresh}>
        ...
        {cardNeeded(s) && (
          <View>
            <View style={t.nh}>
              ...
              <Text style={t.nhVal}>{`${num(s.card_boxes)} of ${num(s.card_target)}`}</Text>
            </View>
            <Bar fill={s.card_target ? (s.card_boxes || 0) / s.card_target : 0} style={{ marginTop: 8 }} />
          </View>
        )}
        <View>
          <View style={t.nh}>
      ...
      <Gap size="s" />
      <Know title="Gold" body={r ? `Fill a Care Card and earn ${num(r.gold_achieve)} Tier Points.` : 'Fill a Care Card and earn Tier Points.'} />
      <Know title="Black" body={r ? `Fill a Care Card and earn ${num(r.black_achieve)} Tier Points.` : 'Fill a Care Card and earn Tier Points.'} />
      ...
      <Kinds>
        ...
        <Kind label="Tier Points" text={r ? `1 Tier Point for each £1 you spend. ${num(r.gold_achieve)} for Gold and ${num(r.black_achieve)} for Black.` : '1 Tier Point for each £1 you spend.'} />
      </Kinds>
      ...
      <Copy>
        {r
          ...
          : 'To stay in your tier, during that next year fill a Care Card and earn Tier Points. If not, on 1 January you move to the tier your year supports.'}
      </Copy>
      {r && (
        <>
          <Gap size="s" />
      ...
      <Copy>
        {r
          ? `A maintenance of £${num(r.care_visit_min_pounds)} or more, at least ${num(r.gap_weeks_tapes)} weeks after your last one for tapes, or ${num(r.gap_weeks_other)} weeks for everything else.`
          : 'A maintenance that fills a box on your Care Card and counts towards your tier.'}
      </Copy>

      <Gap />
      <Sect>Joining in 2026</Sect>
      <Gap size="s" />
      <Copy>Everything from the day you join counts towards 2027.</Copy>
    </Screen>
  );
}
