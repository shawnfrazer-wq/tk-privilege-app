...
// 1 OPENING SCREEN, black then the loop, then the button rises over it.
// It plays every time the app opens and never moves on by itself: she always presses the button.
// Signed out, Log in goes to Your Mobile Number. Signed in within her 90 days, Enter goes straight to
// Home (or to Your Details while her card is incomplete). app_link runs in the background meanwhile.
...
export default function Opening() {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.timing(enter, { toValue: 1, duration: 900, delay: BUTTON_DELAY_MS, easing: EASE, useNativeDriver: true }),
    ]).start();
  }, [mark, sub, vid, enter]);
  ...
  return (
    <View style={s.screen}>
      ...
      <Animated.View style={[s.enter, rise(enter, 18)]}>
        {signedIn ? (
          <Btn variant="light" label="Enter" onPress={() => router.replace(gate ? '/details' : '/home')} />
        ) : (
